...
// The hardware op runs first: any NaN operand propagates to a NaN result, so
// one compare of the result decides whether the positional walk is needed.
// NaN results are never canonicalised (FPCR.DN is clear, so the hardware
// default QNaN is PPC's), and adding a NaN to itself quiets it without
// changing its payload.
template <typename Reg>
static void EmitPpcNanWalk(A64Emitter& e, const Reg& dest, const Reg* order,
                           int count, Xbyak_aarch64::Label& done) {
  for (int k = 0; k < count; ++k) {
    auto& not_nan = e.NewCachedLabel();
    e.fcmp(order[k], order[k]);
    e.b_near(VC, not_nan);
    e.fadd(dest, order[k], order[k]);
    e.b(done);
    e.L(not_nan);
  }
}
...
static void EmitFpBinOpWithPpcNan(A64Emitter& e, std::type_identity_t<Reg> dest,
  ...
  e.fcmp(dest, dest);
  e.b_near(VC, done);
  // First NaN by position wins; with no NaN operand dest keeps the result.
  const Reg order[2] = {t1, t2};
  EmitPpcNanWalk(e, dest, order, 2, done);
  e.L(done);
}

// PPC FMA NaN selection (PowerISA 4.6.7.2):
// The first NaN operand by position (frA=s1, frC=s2, frB=s3) wins,
// regardless of QNaN vs SNaN, and is quieted. A NaN generated by the
// operation itself (0*inf, inf-inf) is the default QNaN.
...
static void EmitFmaWithPpcNan(A64Emitter& e, std::type_identity_t<Reg> dest,
  ...
  e.fcmp(dest, dest);
  if (negate) {
    // Numeric results only: every NaN takes the branch, and exits below.
    auto& nan_result = e.NewCachedLabel();
    e.b_near(VS, nan_result);
    e.fneg(dest, dest);
    e.b(done);
    e.L(nan_result);
  } else {
  ...
  const Reg order[3] = {t1, t3, t2};
  EmitPpcNanWalk(e, dest, order, 3, done);
  e.L(done);
}
