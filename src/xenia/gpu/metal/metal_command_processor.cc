...
void MetalCommandProcessor::InitializeResidencySet() {
  ...
  residency_set_supported_ =
      MetalObjectRespondsToSelector(
          device_, "newResidencySetWithDescriptor:error:") &&
...
void MetalCommandProcessor::RegisterInitialResidencySetResources() {
  AddResidencySetResource(shared_memory_ ? shared_memory_->GetBuffer()
                                         : nullptr);
  AddResidencySetResource(render_target_cache_
...
bool MetalCommandProcessor::TryReuseConstantPayloadFromCache(
  ...
  if (!cvars::metal_constant_payload_cache) {
    return false;
  }
  if (!payload || !size) {
    return false;
...
bool MetalCommandProcessor::PopulateBindlessTables(
  ...
  auto uniform_cbv_identity = [&](size_t stage, size_t cbv) {
    ...
    return RootSlotIdentity{true, true, uniform_cbv.buffer,
                            uniform_cbv.offset};
  };
  ...
  auto select_common_uniform_cbv_identity = [&](size_t cbv) {
    RootSlotIdentity vertex_identity =
        uniform_cbv_identity(kStageVertex, cbv);
  ...
  auto set_root_slot = [&](size_t slot, uint64_t gpu_address,
    ...
    const bool slot_changed =
        entries_were_initialized ? previous_gpu_address != gpu_address
                                 : gpu_address != 0;
    if (!slot_changed) {
  ...
  set_cbv_slot(kGraphicsRootABSlotCBVVertexFloat, kStageVertex,
               kCbvSlotFloat);
  set_cbv_slot(kGraphicsRootABSlotCBVVertexFetch, kStageVertex,
               kCbvSlotFetch);
  ...
  if (use_tessellation_emulation) {
    ...
    set_cbv_slot(kGraphicsRootABSlotCBVHullFloat, kStageVertex,
                 kCbvSlotFloat);
    set_cbv_slot(kGraphicsRootABSlotCBVHullFetch, kStageVertex,
                 kCbvSlotFetch);
  ...
  const bool root_valid =
      root_state.valid && root_state.allocation.valid &&
...
void MetalCommandProcessor::MaybeDumpBackendTelemetry(const char* reason,
  ...
  const std::string shared_memory_upload_route_bytes = MetalFormatNamedCounts(
      ...
      MetalSharedMemoryUploadRouteName);
  const std::string shared_memory_direct_write_rejects =
      MetalFormatNamedCounts(
  ...
  const std::string texture_upload_compatibility_bytes =
      MetalFormatNamedCounts(
          backend_telemetry_.texture_upload_compatibility_bytes,
          MetalTextureUploadCompatibilityClassName);
  const std::string texture_upload_compute_blockers =
      MetalFormatNamedCounts(
  ...
  const std::string texture_upload_compute_blocker_bytes =
      MetalFormatNamedCounts(
          ...
          MetalTextureUploadComputeBlockerName);
  const std::string texture_upload_execution_details =
      MetalFormatNamedCounts(
  ...
  const std::string render_resource_registry_builds =
      MetalFormatNamedCounts(backend_telemetry_.render_resource_registry_builds,
                             MetalTelemetryRenderResourceSetName);
  const std::string render_resource_registry_registers =
      MetalFormatNamedCounts(
...
uint64_t MetalCommandProcessor::GetBindlessTextureResourceSourceSerial()
    const {
...
uint64_t MetalCommandProcessor::GetBindlessRootResourceSourceSerial(
  ...
  hash_value(reinterpret_cast<uintptr_t>(
      allocation.valid ? allocation.buffer : nullptr));
  for (size_t stage = 0; stage < kStageCount; ++stage) {
...
void MetalCommandProcessor::PublishBindlessFixedResourceSet(
    MTL::ResourceUsage shared_memory_usage) {
  constexpr size_t kFixedKindIndex =
      size_t(RenderResourceSetKind::kFixed);
...
void MetalCommandProcessor::PublishBindlessTextureResourceSet() {
  constexpr size_t kTextureKindIndex =
      size_t(RenderResourceSetKind::kTexture);
...
void MetalCommandProcessor::PublishBindlessRootResourceSet(
    const UniformBufferInfo& uniforms) {
  constexpr size_t kRootKindIndex =
      size_t(RenderResourceSetKind::kRoot);
